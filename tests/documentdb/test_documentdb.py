...
from toucan_connectors.documentdb.documentdb_connector import (
    DocumentDBConnector,
)
...
def test_username_password():
    # password not set
    mongo_connector = DocumentDBConnector(name='mycon', host='localhost', port=22)
    assert mongo_connector._get_mongo_client_kwargs() == {'host': 'localhost', 'port': 22}

    # password set to None
    mongo_connector = DocumentDBConnector(name='mycon', host='localhost', port=22, password=None)
    assert mongo_connector._get_mongo_client_kwargs() == {'host': 'localhost', 'port': 22}

    # password set without username
    with pytest.raises(ValueError) as e:
        DocumentDBConnector(name='mycon', host='localhost', port=22, password='bibou')
    assert 'username must be set' in str(e.value)

    # password and user set
    mongo_connector = DocumentDBConnector(
...
def test_client_with_detailed_params():
    connector = DocumentDBConnector(name='my_mongo_con', host='myhost', port='123')
    assert isinstance(connector.client, pymongo.MongoClient)


def test_client_with_mongo_uri():
    connector = DocumentDBConnector(name='my_mongo_con', host='mongodb://myuser:mypassword@myhost:123')
    assert isinstance(connector.client, pymongo.MongoClient)
