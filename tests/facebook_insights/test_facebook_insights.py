            ...
            'data': [
                {
                    ...
                    'values': [
                        {'end_time': '2019-01-02T08:00:00+0000', 'value': 24},
                        {'end_time': '2019-01-03T08:00:00+0000', 'value': 21},
                    ],
                }
            ]
        },
            ...
            'data': [
                {
                    ...
                    'period': 'week',
                    'title': 'Weekly Total: total action count per Page',
                    'values': [{'end_time': '2019-02-01T08:00:00+0000', 'value': 10}],
                }
            ]
        },
    )
...
def test_get_page_tokens():
    responses.add(
        ...
        json={
            ...
            'paging': {'cursors': {'after': 'NzgwODkyODA1NjMwODU0', 'before': 'MjkyMDc0MTQ3MzAw'}},
        },
    )
    tokens = get_page_tokens('the-longlived-token')
    assert tokens == {'page-id-1': 'access-token-1', 'page-id-2': 'access-token-2'}
...
def test_get_longlived_tokens():
    responses.add(
        ...
        content_type='application/json',
        json={'access_token': 'the-longlived-token', 'token_type': 'bearer'},
    )
...
def test_get_facebook_insights_df():
    ...
    datasource = FacebookInsightsDataSource(
        name='test',
        domain='test',
        pages={'page-id-1': 'access-token-1', 'page-id-2': 'access-token-2'},
        metrics=['page_total_actions'],
    )
    connector = FacebookInsightsConnector(name='facebook', type='facebook_insights')
