...
from toucan_connectors.aircall.aircall_connector import AircallConnector, AircallDataSource, fetch_page
...
async def test_fetch_page_with_no_next_page(mocker):
    loop = asyncio.get_event_loop()
    session = aiohttp.ClientSession()
    with aioresponses() as aiomock:
        # base_query = f'https://{STUFF}api.aircall.io/v1/users'
        base_query = 'https://eg.com/users'
        aiomock.get(f'{base_query}', status=200, payload={
            'data' : 'Hai World!',
            'meta' : {
                'next_page_link' : None
            }
        })
        res = loop.run_until_complete(fetch_page(base_query, [], session, 10, 1))
        loop.close()
