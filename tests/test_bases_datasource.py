...
class DataSource(ToucanDataSource):
    ...
    test_default: int = 101  # not required because it has a default, validated against type when present


def test_instantiation():
    # no errors with required args at the right type
    data_source =  {'domain': 'my_domain', 'name': 'my_name', 'collection': 'my_collection', 'query': {}}
...
def test_required_arg_wrong_type():
    ...
    data_source =  {'domain': 1, 'name': 'my_name', 'collection': 'my_collection', 'query': {}}
    with pytest.raises(BadParameters) as e:
        mds = DataSource(**data_source)
    assert e.value.args[0][0][0] == 'domain'
...
def test_not_required():
    data_source = {'domain': 'my_domain', 'name': 'my_name', 'collection': 'my_collection', 'query': {}, 'comment': 'test'}
    mds = DataSource(**data_source)
    assert mds.comment == 'test'


def test_default_override():
    data_source = {'domain': 'my_domain', 'name': 'my_name', 'collection': 'my_collection', 'query': {}, 'test_default': 102}
    mds = DataSource(**data_source)
    assert mds.test_default == 102


def test_default_override_validated():
    data_source = {'domain': 'my_domain', 'name': 'my_name', 'collection': 'my_collection', 'query': {}, 'test_default': '1'}
    with pytest.raises(BadParameters) as e:
        mds = DataSource(**data_source)
...
def test_unknown_arg():
    data_source = {'domain': 'my_domain', 'name': 'my_name', 'collection': 'my_collection', 'query': {}, 'unk': '@'}
    with pytest.raises(BadParameters) as e:
        mds = DataSource(**data_source)
