...
from toucan_connectors.salesforce_sandbox.salesforce_sandbox_connector import (
    NoCredentialsError,
    SalesforceApiError,
    SalesforceSandboxConnector,
)

import_path = 'toucan_connectors.salesforce_sandbox.salesforce_sandbox_connector'
...
def test_get_status_api_down(mocker, sc):
    ...
    """
    mocker.patch.object(SalesforceSandboxConnector, 'get_access_data', side_effect=HttpError)
    assert sc.get_status().status is False
...
def test_get_status_ok(mocker, sc):
    ...
    """
    mocker.patch.object(
        SalesforceSandboxConnector, 'get_access_data', return_value={'access_token': 'access_token'}
    )
    assert sc.get_status().status is True


def test_get_status_nok(mocker, sc):
    mocker.patch.object(SalesforceSandboxConnector, 'get_access_data', return_value=None)
    assert sc.get_status().status is False
...
def test_generate_rows_error(mocker, sc, ds, error_result):
    ...
    mocker.patch.object(SalesforceSandboxConnector, 'make_request', return_value=error_result)
...
def test__retrieve_data(mocker, sc, ds, clean_p1):
    ...
    secret_object = {
        ...
        'id': 'https://test.salesforce.com/id/00D09000007vcxHEAQ/00509000006bXeyAAE',
        'token_type': 'Bearer',
        'issued_at': '1621949493610',
        'refresh_token': 'shiny token',
    }
    mocker.patch.object(SalesforceSandboxConnector, 'get_access_data', return_value=secret_object)
    mocked_generate_rows = mocker.patch.object(
        SalesforceSandboxConnector, 'generate_rows', return_value=clean_p1
    )
...
def test_get_secrets_form(mocker, sc):
    ...
    mocker.patch(
        'toucan_connectors.salesforce_sandbox.salesforce_sandbox_connector.os.path.dirname',
